...
class NoneTest {
    ...
    fun updateConfig() {
        ApplicationProvider.getApplicationContext<PetBaseTestApp>().config =
            Config(false, false, "M-F 8:00-16:00")
        activityRule.activity.updateConfig()
    }

    @Test
    fun checkButtonVisibility() {
        onView(withId(R.id.chatButton)).check(matches(withEffectiveVisibility(GONE)))
        onView(withId(R.id.callButton)).check(matches(withEffectiveVisibility(GONE)))
    }
}
